...
#ifdef DEBUG
#define CYLog(value) fprintf(stderr,"line:%d function:%s %s = %s\n", __LINE__, __FUNCTION__, (#value), [_CYLog(@encode(__typeof__((value))), (value)) UTF8String])
#else
#define CYLog(value)
#endif

static inline NSString* _CYLog(const char *type, ...)
{
    va_list v;
    va_start(v, type);
    NSNumber *number;
